...
from app.models import Blog, Listing, RequestSubmission, User
...
# Contact route with email functionality
...
def log_submission(name, email, subject, message):
    ...
    log_entry = (
        f"[{timestamp}] {name} <{email}> | Subject: {subject} | Message: {message}\n"
    )
...
def contact():
    ...
    try:
        ...
        with smtplib.SMTP_SSL("mail.ilikeitproperties.co.ke", 465) as smtp:
            ...
            smtp.send_message(
                admin_msg,
                from_addr="info@ilikeitproperties.co.ke",
                to_addrs=["info@ilikeitproperties.co.ke"],
            )
            smtp.send_message(
                reply_msg, from_addr="info@ilikeitproperties.co.ke", to_addrs=[email]
            )
...
def new_blog():
    ...
    if request.method == "POST":
        ...
        image_url = (
            upload_to_r2(image_file)
            if image_file and image_file.filename
            else "https://via.placeholder.com/600x400?text=No+Image"
        )
...
def edit_listing(id):
    ...
    if request.method == "POST":
        ...
        if files and any(f.filename for f in files):
            ...
            new_images = [
                upload_to_r2(file) for file in files if file and file.filename
            ]
